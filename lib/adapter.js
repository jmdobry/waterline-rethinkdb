		...
		if (single) {
					...
					function (next) {
						_this.connectionRun(r.table(collectionName).get(options.where[collection.primaryKey || 'id']).update(values, { returnVals: true }), function (err, cursor) {
							if (err) {
								...
								opts.where[collection.primaryKey || 'id'] = cursor['new_val'][collection.primaryKey || 'id'];
								// Call the find method so the updated row is folded into a Model instance.
								// DOESN'T WORK
								collection.find(collectionName, opts, next);
							}
						});
					}
				], cb);
			...
			async.waterfall([
				...
				if (err) {
					...
					// Each 2nd-level call to adapter.update(...) returned a single-item array.
					// We must turn this array of arrays to an array of the single-items.
					if (result) {
