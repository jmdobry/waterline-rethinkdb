		...
		createEach: function (collectionName, values, cb) {
			...
			this.run(r.table(collectionName).insert(values), function (err, result) {
				if (err) {
					...
					// Alternate query: r.expr(result.generated_keys).map(r.table(collectionName).get(r.row))
					// Which is better?
					var query = r.table(collectionName);
					self.run(query.getAll.apply(query, result.generated_keys), function (err, cursor) {
						if (err) {
							cb(err);
						} else {
