...
module.exports = (function () {
	...
	var RethinkDBAdapter = {
		...
		update: function (collectionName, options, values, cb) {
			console.log('update(collectionName, options, values, cb)', collectionName, options, values)
			async.waterfall([
				function (next) {
					getConnection({}, next);
				},
		...
		destroy: function (collectionName, options, cb) {
			console.log('destroy(collectionName, options, cb)', collectionName, options);
			async.waterfall([
				function (next) {
					getConnection({}, next);
				},
				function (conn, next) {
					if (options.where) {
						if (options.where.id) {
							r.table(collectionName).get(options.where.id).delete().run(conn, function (err, cursor) {
								err ? cb(err) : next(null);
							});
						} else {
							cb('not supported');
						}
